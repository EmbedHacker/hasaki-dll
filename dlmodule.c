...
size_t my_strcmp(const char *cs, const char *ct)
{
    while (*cs && *cs == *ct)
        cs++, ct++;
...
void* dlsym(void *handle, const char *symbol)
{
    ...
    module = (dloader_p)handle;
 
    for(i=0; module->functab->num; i++)
    {
        if (my_strcmp(module->functab->symtab[i], symbol) == 0)
            return (void*)(module->functab->user_info[i]);
    }
